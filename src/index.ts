...
client.on("message", async (message) => {
  ...
  if (db.has(`prefixes_${guild.id}`)) prefix = db.get(`prefixes_${guild.id}`);
  ...
  if (command === "prefix") {
    ...
    if (args[0] === "set") {
      ...
      if (!newP) return message.channel.send("You did not mention a prefix");
  ...
  if (command === "backup") {
    ...
    if (args[0] === "info") {
      const ID = args[1];
      if (!ID) return message.reply("You did not given an ID");

      if (!db.has(ID)) return message.channel.send("Unknown backup");

      let data = db.get(ID);

      if (!data) return message.channel.send(`Failed to fetch the backup`);
    ...
    if (args[0] === "delete") {
      let ID = args[1];
      if (!ID) return message.reply("You did not give an ID");

      if (!db.has(ID)) return message.channel.send("That ID doesn't exist");

      let data = db.get(ID);

      if (!data) return message.reply("I failed to fetch the backup");
    ...
    if (args[0] === "create") {
      if (!message.member.hasPermission("ADMINISTRATOR"))
        return message.channel.send(
          "You need ADMINISTRATOR permissions to create a backup"
        );
      ...
      values.forEach((value) => {
        if (value.ID) {
          if (value.owner === message.author.id)
            x.push(
              `${value.ID} - ${value.guildName} - ${
                value.date || "Failed to find date"
              }`
            );
        }
      });
    ...
    if (args[0] === "load") {
      if (!message.member.hasPermission("ADMINISTRATOR"))
        return message.channel.send(
          "You need ADMINISTRATOR permissions to load a backup"
        );
      const ID = args.slice(1).join(" ");

      if (!ID) return message.channel.send("You did not give an ID");

      if (!db.has(ID)) return message.reply("That id doesn't exist");
